...
public class Review extends JPanel {
	...
	JPanel listTask() {
		...
		for (ReviewTask k : lt) {
			...
			rev5.addActionListener(e -> {
				if (k.getState() == 1) {
					...
					if (k.getProgressNow() + 1 == k.getSetProgress()) {
						...
						int op = JOptionPane.showOptionDialog(null, "恭喜您！本次复习任务已完成", "结束任务", JOptionPane.YES_NO_OPTION,
								JOptionPane.QUESTION_MESSAGE, null, ending, ending[0]);
						if (op == 0) {
							k.reset();
							centerPanel.removeAll();
							centerPanel.repaint();
			...
			geJp.add(rtJp2);
			geJp.add(rtJp3);
			geJp.setPreferredSize(new Dimension(780, geJp.getPreferredSize().height));
			geJp.setMaximumSize(new Dimension(780, geJp.getPreferredSize().height));
			geJp.setMinimumSize(new Dimension(780, geJp.getPreferredSize().height));
